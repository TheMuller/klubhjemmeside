	...
	foreach($EventOrders as $kEO=>$vEO)
	{
		...
		//if(elgg_is_admin_logged_in()){
            for($i=1;$i<=5;$i++)
            {
               ...
               if($xTYP and  $vEO->status =='processing')
                {
...
if($user_total_spots >0) {
    $body .='<CX>'.elgg_echo('event_calendar:ticket:usertotalspot')."<CXspan>".$user_total_spots.'</CXspan></CX>';
    $tktleft = $tktleft - $user_total_spots;
    if($tktleft >0){
        $body .='<CX>'.elgg_echo('event_calendar:ticket:usertotalleft').'<CXspan>'.$tktleft.'</CXspan></CX>';
    }else{
     $body .='<CX>'.elgg_echo('event_calendar:ticket:usertotalconsumed').'&nbsp;</CX>';
    }
    $body .='<br clear="all">';
}
...
$body.="";
if(elgg_is_admin_logged_in()){
	$body.="<ul class='me_ul_as_table'><li id='table_header' class ='me_div_as_th'><div class ='me_div_as_td' style='width:200px;'>".elgg_echo('event_calendar:ticket:menu:type')."</div><div class ='me_div_as_td' style='width:80px;'>".elgg_echo('event_calendar:ticket:menu:price')."</div><div class ='me_div_as_td' style='width:120px;'>".elgg_echo('event_calendar:ticket:menu:select')."</div><div class ='me_div_as_td' style='width:90px;'>".elgg_echo('event_calendar:ticket:menu:meximum_spot')."</div><div class ='me_div_as_td' style='width:70px;'>".elgg_echo('event_calendar:ticket:menu:total_tickets')."</div><div class ='me_div_as_td' style='width:50px;'>".elgg_echo('event_calendar:ticket:menu:sold_tickets')."</div><div class ='me_div_as_td' style='width:60px;'>".elgg_echo('event_calendar:ticket:menu:left_tickets')."</div></li>";
}else{
	$body.="<ul class='me_ul_as_table'><li id='table_header' class ='me_div_as_th'><div class ='me_div_as_td' style='width:200px;'>".elgg_echo('event_calendar:ticket:menu:type')."</div><div class ='me_div_as_td' style='width:100px;'>".elgg_echo('event_calendar:ticket:menu:price')."</div><div class ='me_div_as_td' style='width:130px;'>".elgg_echo('event_calendar:ticket:menu:select')."</div><div class ='me_div_as_td' style='width:90px;'>".elgg_echo('event_calendar:ticket:menu:meximum_spot')."</div><div class ='me_div_as_td' style='width:70px;'>".elgg_echo('event_calendar:ticket:menu:sold_tickets')."</div><div class ='me_div_as_td' style='width:80px;'>".elgg_echo('event_calendar:ticket:menu:left_tickets')."</div></li>";
}
...
$body.="<li class='me_li_as_tr'>";
    ...
    if($ticket_type)//&& $ticket_amount)
    {
        ...
        if($ticket_spots==0)
        {
			...
			///////////////////////////////   Aj 09-05-14 for (if amount = 0)
				if($ticket_amount_X == 0){
					$sold_spot = $grand_total_spot['ticket_type_' . $i];
					if(elgg_is_admin_logged_in()){
						$left_spot = $ticket_spots - $sold_spot;
						$body.='<div class=me_div_as_td>&nbsp;'.$ticket_spots.'</div>';
						}	
					$body.=		'<div class=me_div_as_td>&nbsp;'.$sold_spot.'</div><div class=me_div_as_td>&nbsp;&nbsp;'.$left_spot.'</div><br clear="all">';
				}else{
				/////////////////////
					$sold_spot = $grand_total_spot['ticket_type_' . $i];
					if(elgg_is_admin_logged_in()){
