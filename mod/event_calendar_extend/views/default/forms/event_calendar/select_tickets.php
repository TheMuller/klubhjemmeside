...
$PrevTO='<b>'.elgg_echo('event_calendar:prev_to').'</b><br><br>';
$vEO_CTRS=array();
$vEO_CTRZ=array();
$TIME=time();
$TM24=time()-86400;
...
//````````````````````````````````````````
			//:DC:
			echo"<style>
			C0{width:300px;border:dotted red    0px;display:block;float:left;}
	/**/	C1{width:132px;border:dotted orange 0px;display:block;float:left;text-align:right;}
			C2{width:125px;border:dotted red    0px;display:block;float:left;text-align:right;}
			C3{width:040px;border:dotted red    0px;display:block;float:left;}
	...
	/**/	C5{width:132px;border:dotted blue   0px;display:block;float:left;text-align:right;}
			C6{width:400px;border:dotted red    0px;display:block;float:left;}
...
//		..._extend/	select_tickets.php
	...
	{	//:DC:
		//````````````````````````````````````````
		$body.=	$PrevTO
			.'<div style="border-top:dotted #CCC 1px;height:0px;clear:both;margin-bottom:-1px;"></div>'
			.''
			.'<i><C0><b>'.elgg_echo('event_calendar:prev_to_1').'</b> '.$kEO.'&nbsp; </C0>'
			.'<C4>&nbsp;</C4><C1>'.number_format($OrderTotal[$kEO],2,',','.').'&nbsp;DKK&nbsp;</C1>'
//:DC:	'awaitingpayment' language text!
//````````````````````````````````````````
			.' &nbsp; <small><small>'.elgg_echo($OrderStatus[$kEO])
			.' &nbsp; '.$OrderCreated[$kEO]
			//.' ` AGED='.$OrderAge[$kEO]
			.'</small></small>'
			.'</i>'
		...
		$body.='<div '.'style="width:11%;float:right;text-align:right;">'
			.'&nbsp;'
			.elgg_view(
				'output/confirmlink',
				array(
...
//````````````````````````````````````````
	...
	$body.='<br>';
	$body.='<p><b>'.elgg_echo('event_calendar:ticket:list').':</b><br><CZ></CZ>';//:DC:
	$spots_counter = 0;
	$doublecheckCTR=0;
	//````````````````````````````````````````
	for($i = 1; $i <=5; $i++)				//:DC:
