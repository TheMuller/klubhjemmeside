...
if (!$owner) {
	forward('calendarview/all');
}

// set breadcrumb
elgg_push_breadcrumb(elgg_echo('calendarview'), "calendarview/all");
elgg_push_breadcrumb($owner->name, "calendarview/owner/$owner->username");
elgg_push_breadcrumb(elgg_echo('friends'));

// set title
$title = elgg_echo("calendarview:friends");
...
$content .= '<div id="nav">';
$content .= '<form action="'.$CONFIG->url.'calendarview/friends/'.$owner->name.'" method="post">';
$content .= '<input type="text" name="year" size="4" maxlength="4" value="'.$year.'">';
...
$content .= '</select>';
$content .= '<input type="submit" value="'.elgg_echo('calendarview:go').'">';
$content .= '</form>';
$content .= '</div>';
...
// get user friends
$friends = get_user_friends($owner->guid,"",5000);
if ($friends) {
        $friend_guids = array();
