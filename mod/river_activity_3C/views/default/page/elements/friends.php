...
$friend = elgg_list_entities_from_relationship(array(
	...
	if ($friend) {
		$river_body = $friend;
                $river_body .= '<p style="text-align:right; margin:3px 3px;"><a href="'.$vars["url"].'friends/'.$_SESSION['user']->username.'"><b>'.elgg_echo('river_activity_3C:viewmore').'</b></a></p>';
	}else {
            
            $river_body = elgg_echo ('river_activity_3C:friends-no');
            $river_body .= '<p style="text-align:right; margin:3px 3px;"><a href="'.$vars["url"].'members"><b>'.elgg_echo('river_activity_3C:addfriends').'</b></a></p>';
        }
    echo elgg_view_module('dropdown', $title, $river_body);
?>
