...
$box_view = elgg_get_plugin_setting('view_riverbox', 'river_activity_3C');
        ...
        if ($membership_groups) {
                ...
                elgg_pop_context();
                $river_body .= '<p style="text-align:right; margin:3px 3px;"><a href="'.$vars["url"].'groups/member/'.$_SESSION['user']->username.'"><b>'.elgg_echo('river_activity_3C:viewmore').'</b></a></p>';
                echo elgg_view_module($box_view, $title, $river_body);
        }else {
           $river_body = elgg_echo ('river_activity_3C:groupsmember-no');
           $river_body .= '<p style="text-align:right; margin:3px 3px;"><a href="'.$vars["url"].'groups/all"><b>'.elgg_echo('river_activity_3C:join').'</b></a></p>';
           echo elgg_view_module($box_view, $title, $river_body);
        } 
