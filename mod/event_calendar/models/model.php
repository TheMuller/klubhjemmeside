...
{
//echo'<h2>(2) ==>ORDER_BY<==',$order_by,'</h2>';
	...
	$limit = (int)$limit;
	$offset = (int)$offset;
	//if ($order_by == "") $order_by = "e.time_created desc";	//:DC:order_by
	if ($order_by == "") $order_by = "v.string asc";
	$order_by = sanitise_string($order_by);
	$site_guid = (int) $site_guid;
	if ((is_array($owner_guid) && (count($owner_guid)))) {
	...
	$where[] = "m.name_id='$meta_start_n'";
	$where[] = "m2.name_id='$meta_end_n'";
	if($meta_start_value==0)
		{
		$where[] = "
		(
				(v2.string >= $meta_start_value AND v2.string <= $meta_end_value)
		)";
		}
	else
		{
		$where[] = "
		(
				(v.string  >= $meta_start_value AND v.string  <= $meta_end_value)
			OR	(v2.string >= $meta_start_value AND v2.string <= $meta_end_value)
			OR	(v.string  <= $meta_start_value AND v2.string >= $meta_start_value)
			OR	(v2.string <= $meta_end_value   AND v2.string >= $meta_end_value)
		)";
		}
	if ($region && $region != '-') {
		$where[] = "m3.name_id='$region_n'";
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//````````````````````````````````````````````````````````````````````````````````````````````````````	
// pages			//:DC: ++PAST EVENTS	
//````````````````````````````````````````````````````````````````````````````````````````````````````	
function event_calendar_get_page_content_list
	(				//:DC: ++PAST EVENTS
		$page_type,
		$container_guid,
		$start_date,
	...
	//````````````````````````````````````````````````````````````````````````````````````````````````````	
	{
//echo"<br>:event_calendar_get_page_content_list:";
...
////////////////////////////////////////
		...
		//````````````````````````````````````````````````````````````````````````````````````````````````````	
		$event_generate_params = event_calendar_generate_listing_params
		(					//??:DC:?
			$page_type,
			$container_guid,
			$start_date,
		...
		//````````````````````````````````````````````````````````````````````````````````````````````````````	
		$body = elgg_view_layout("content", $event_generate_params);
...
//	````````````````````````````````````````````````````````````````````````````````
//	````````````````````````````````````````````````````````````````````````````````
function event_calendar_generate_listing_params(
	$page_type,
	$container_guid,
	$original_start_date,
	$display_mode,
...
/**				echo"
PARAMS:									
	T	$page_type						<!--	C	$container_guid					-->
	S	$start_date						<!--	M	$display_mode					-->
	F	$filter							<!--	R	$region							-->
>>>>>>>>V	$pastevents	";							
**/
				//exit();//	`	`	`	`	`	`	`	`	`	`
				//````````````````````````````````````````````````````````````````````````````````````````````````````	
				$start_date0=time($start_date);
/**				echo"
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
?	BEFORE:	FMT ($event_calendar_listing_format)
	ST0 ($start_date0)	STT ($start_date)
	END ($end_date)
>>>>>>>>PAS ($pastevents)	";
**/				$event_calendar_listing_format='paged';		//	1354076856	EN2
				$start_date	=0;								//	1353708050	EVT
				$start_date2=strftime('%Y-%m-%d',$start_date);
				$end_date	=time()-(86400*1);//:DC: *1 b/c metadata call fetch is *inclusive
				$end_date2	=$end_date;	
				$end_date2	=strftime('%Y-%m-%d',$end_date);
				$timenow	=time();
				$time_now	=strftime('%Y-%m-%d',$timenow);
				$hour_now	=strftime('%Y-%m-%d %H:%I:%S',$timenow);
//````````````````````````````````````````
//			..._standard/	model.php
//:DC:	debug stuff!
				/**
...
AFTER
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
*	FMT ($event_calendar_listing_format)
	STT ($start_date)			ST2 (",$start_date2,")
	END ($end_date)	EN2 (",$end_date2,")
>>>>>>>>NOW (",$time_now,")	HRS (",$hour_now,")	</pre>";
				echo"</h2>";
**/
			}
//:DC:	:END:
...
//sachin        echo "end_date = ".$end_date;
		...
		if ($filter == 'pastevents')				//:DC:
			{
			//$order_by='e.time_created desc';			//:DC:
			$order_by='v.string desc';					//:DC:
			//echo'<h2>(1) ==>ORDER_BY<==',$order_by,'</h2>';
				$count  = event_calendar_get_events_between($start_ts,$end_ts,true, $limit,$offset,$container_guid,$region		
					,$order_by);
				$events = event_calendar_get_events_between($start_ts,$end_ts,false,$limit,$offset,$container_guid,$region
					,$order_by);
	...
	//	````````````````````````````````````````````````````````````````````````````````
	//	````````````````````````````````````````````````````````````````````````````````
 	//TRAP WIDGET DATA HERE?
	//

		$vars = array(				//:DC: ++PAST EVENTS
		...
		'filter'		=> $filter,
		'region'		=> $region,
		'listing_format'=> $event_calendar_listing_format,	//:DC:
		);
							//:DC: ++PAST EVENTS
							//:DC: ???????
	/**
	echo"<h2>K::GOTO ~~~===></h2>";
	echo"<pre><Xh2>\$VARS ['EVENTS'] ::>";
	print_r($vars['events']);
	echo"</Xh2></pre>";
	**/
	/**
	echo"<h2>K::GOTO ~~~> SHOW_EVENTS.PHP</h2>";
	echo"<h2>
		(
			STTS ($start_ts)
			ENTS ($end_ts)
	...
	**/
	...
	$vars['end_date']  =$end_date;		//:DC: ++PAST EVENTS
	//	````````````````````````````````````````````````````````````````````````````````
	//	````````````````````````````````````````````````````````````````````````````````
	/**
	echo"<pre>";
	echo"&nbsp; ` &nbsp; ` &nbsp; ` &nbsp; ` ";
	echo"<span style='color:blue;'>AFTER:: \$content = elgg_view('event_calendar/show_events', \$vars);";
	echo"<br>";
	echo"&nbsp; ` &nbsp; ` &nbsp; ` &nbsp; ` ";
	echo"=:> ",strlen($content);
	echo"</span>";
	echo"</pre>";
	echo"<hr size=6 color=blue noshade>";
	**/
	//echo"",htmlspecialchars($content);
	//echo"<hr size=6 color=red noshade>";
	//	````````````````````````````````````````````````````````````````````````````````
...
{
	//echo"<h2>@ 0 X event_calendar_get_page_content_display_users::($event_guid)::</h2><hr>";//:DC:
	elgg_load_js('elgg.event_calendar');
	$event = get_entity($event_guid);
	if (!elgg_instanceof($event, 'object', 'event_calendar'))
	{
